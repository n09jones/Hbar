...
void generate_ICs(int argc, char ** argv, size_t tot_parts, size_t part_idx)
{
    ...
    for(size_t IC_idx = 0; IC_idx < kh.ICs.size(); ++IC_idx)
    {
        ...
        std::cout <<  "Minimum B : " << aseh.ill.B_0_det << std::endl;
	std::cout << "Detuning : " << aseh.ill.det_0 << std::endl;
	std::cout << "rel_det : " << aseh.ill.det << std::endl;
        ...
        std::cout << "One-photon decays : " << kh.final_states.back().one_photon_decays << std::endl;
