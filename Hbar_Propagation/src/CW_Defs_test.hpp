...
tot++;
        ...
        for(; ((kh.t < kh.t_lim) && !(kh.out_of_bounds()) && aseh.in_beam(kh.pos_vel)); kh.t += kh.dt)
        {
            ...
            {
std::cout << ++detailed << " of " << tot << ", scatters = " << aseh.dec.one_ph_decays <<  std::endl;
                reset_kh(kh);
                return true;
            }
...
auto tmp_one_ph {aseh.dec.one_ph_decays};
...
if(((aseh.dec.one_ph_decays != tmp_one_ph) || aseh.ill.is_excited()) && !full_propagation) {
std::cout << "Fail!" << std::endl;
}
else if(((aseh.dec.one_ph_decays != tmp_one_ph) || aseh.ill.is_excited()) && full_propagation)
{
...
void generate_ICs(int argc, char ** argv, size_t tot_parts, size_t part_idx)
{
    ...
    {
std::cout << "Starting " << IC_idx << std::endl;
...
std::cout << "Ending " << IC_idx << std::endl;
    }
